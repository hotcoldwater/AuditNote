...
기준서 496개 조문(활성 491개), 기출 서술형 문제 392개를 수록하고 있습니다.
...
![채점 결과 화면](docs/screenshots/grading-result.png)
![기록 화면](docs/screenshots/records.png)
...
채점 로직(`src/lib/scoring.ts`, `src/lib/aiGrading.ts`)에 대한 유닛 테스트를 Vitest로 작성했습니다.

```bash
npm test
```

`npm run test:watch`로 감시 모드 실행도 가능합니다.

현재 커버하는 케이스:

- `deriveResultStatus`: EXCELLENT / CORRECT / REVIEW / WRONG / SKIPPED 점수 구간 경계값 (정답·오답·부분점수 케이스 포함)
- `normalizeScoringResult`: 점수 clamp, 기본 사유 문구, WRONG/SKIPPED 오답노트 강제 등록, REVIEW 복습 권장 플래그
- `isSkippedAnswer`: 빈 답안, "모르겠어요" 계열 표현, 대소문자/문장부호 변형, 반복된 ㅠ/ㅜ 패턴, 정상 서술형 답안 구분
- `gradeAnswer`(AI 채점 fallback): 네트워크 오류, 채점 API 에러 응답, 잘못된 AI 응답 payload 각각에서 규칙 기반 채점으로 정상 대체되는지, 그리고 AI 응답이 정상일 때는 fallback을 타지 않는지 검증 (Supabase/fetch 모킹)

테스트 파일: [tests/scoring.test.ts](tests/scoring.test.ts), [tests/aiGrading.test.ts](tests/aiGrading.test.ts)
